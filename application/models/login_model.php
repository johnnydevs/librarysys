...
class LoginModel
{
    ...
    public function favouriteTotal()
    {
        ...
        $query = $this->db->prepare("SELECT * FROM favourite WHERE user_id = :user_id");
        $query->bindParam(':user_id', $userid);
        $query->execute();

        $fav = array();
        foreach ($query->fetchAll() as $favs) {   
            $fav[$favs->id] = new stdClass();
            $fav[$favs->id]->book_id = $favs->book_id;
        }
        return $fav;
    }
